...
function All() {
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] text-[#495057] px-4 md:px-10 py-12 space-y-16">
      <div className="text-center mb-10 pt-8">
        <h1 className="text-4xl md:text-6xl font-extrabold font-serif text-[#212529] mb-4">
          Explore India's Finest
        </h1>
        <p className="text-[#495057] text-lg md:text-xl max-w-4xl mx-auto leading-relaxed">
      ...
      {loading ? (
        <div className="text-center text-[#0077B6] text-lg animate-pulse">Loading amazing places...</div>
      ) : error ? (
        <div className="text-center text-red-600 text-lg">{error}</div>
      ) : (
        sections.map((section, index) => (
          <div key={index} className="bg-white p-6 md:p-8 rounded-3xl shadow-lg border border-[#E9ECEF]">
            <h2 className="text-3xl md:text-4xl font-bold text-[#023047] mb-8 pb-4 border-b-2 border-[#E9ECEF]">
              {section.title}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 md:gap-12">
              {section.data.length > 0 ? (
                ...
                <p className="col-span-full text-center text-[#495057] text-lg py-10">
      ...
      {selectedItem && (
        <div className="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-white rounded-3xl shadow-2xl border border-[#E9ECEF] max-w-2xl w-full relative p-6 md:p-8">
            ...
            />
            <h2 className="text-3xl font-extrabold text-[#212529] mb-3">{selectedItem.name}</h2>
            <p className="text-[#495057] text-base mb-4 leading-relaxed">{selectedItem.description}</p>
            <div className="flex items-center text-xl font-bold text-[#212529] mb-2">
              <Star size={20} className="text-[#FFD60A] mr-2" fill="currentColor" />
              {selectedItem.rating ? `${selectedItem.rating} / 5` : "N/A"}
            </div>
            <p className="text-[#495057] text-base mb-6 flex items-start">
              <MapPin size={20} className="text-[#0077B6] mr-2 flex-shrink-0 mt-0.5" />
              <span>
