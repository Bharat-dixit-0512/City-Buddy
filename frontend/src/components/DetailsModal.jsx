...
const DetailsModal = ({ item, onClose }) => {
  ...
  const { register: registerClaim, handleSubmit: handleSubmitClaim } =
    useForm();

  if (!item) return null;

  const getDirectionsUrl = `https://www.google.com/maps?q=${item.location?.coordinates[1]},${item.location?.coordinates[0]}`;

  const onEditSubmit = async (data) => {
    try {
      await axios.post(`${API_BASE}/places/${item._id}/suggest-edit`, {
  ...
  const onClaimSubmit = async (data) => {
    try {
      await axios.post(`${API_BASE}/admin/claims`, {
        placeId: item._id,
  ...
  const modalContent = (
    <div className="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full relative max-h-[90vh] grid grid-cols-1 lg:grid-cols-2">
        <button
          className="absolute top-4 right-4 text-gray-500 hover:text-black z-10 bg-white/70 rounded-full p-1"
          onClick={onClose}
        >
          <X size={24} />
        </button>
        <div className="relative h-full min-h-[300px] lg:min-h-0">
          <img
            ...
            alt={item.name}
            className="absolute w-full h-full object-cover rounded-t-2xl lg:rounded-l-2xl lg:rounded-tr-none"
          />
          {authUser && (
            <button
        ...
        <div className="p-6 md:p-8 flex flex-col overflow-y-auto">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-600 mb-4">
            <div className="flex items-center gap-1.5 font-bold text-yellow-500">
              ...
              <span>
                {item.rating?.toFixed(1) || "New"} ({item.reviewCount || 0}{" "}
                reviews)
              </span>
            </div>
          ...
          {item.priceForTwo && (
            <div className="flex items-center gap-2 text-lg text-gray-800 font-medium mb-4">
              ...
              <span>
                ₹{item.priceForTwo.toLocaleString("en-IN")} for two people
              </span>
            </div>
          )}
          {item.pricePerNight && (
          ...
          {showEditForm && (
            <form
              ...
              className="my-4 p-3 bg-gray-50 rounded-lg space-y-2"
            >
              <textarea
