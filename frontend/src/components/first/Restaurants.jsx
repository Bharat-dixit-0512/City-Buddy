...
function Restaurants() {
  ...
  const uniqueCities = [...new Set(restaurants.map((item) => item.city))].sort();
    ...
    <div className="min-h-screen bg-[#F9FAFB] text-[#495057] px-4 md:px-10 py-12">
      <div className="text-center mb-10 pt-8">
        ...
        <p className="text-[#495057] text-base md:text-lg max-w-2xl mx-auto">
          Discover top-rated restaurants and hotels across India. Find the perfect stay with comfort, style, and convenience.
        </p>
      </div>
      ...
      {loading ? (
        <div className="text-center text-[#0077B6] animate-pulse">Loading restaurants & hotels...</div>
      ) : error ? (
      ...
      {selectedRestaurant && (
        <div className="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-white rounded-3xl shadow-2xl border border-[#E9ECEF] max-w-lg w-full relative p-6 md:p-8">
            ...
            <img
              src={selectedRestaurant.image || "https://via.placeholder.com/600x400"}
            ...
            <div className="flex items-center text-lg font-bold text-[#212529] mb-2">
              <Star
                size={18}
                className="text-[#FFD60A] mr-2"
                fill="currentColor"
              />
              {selectedRestaurant.rating ? `${selectedRestaurant.rating} / 5` : "N/A"}
            </div>
